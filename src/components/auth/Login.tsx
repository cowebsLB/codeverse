...
export default function Login() {
  ...
  useEffect(() => {
    if (email) {
      ...
      if (rateLimit.isLimited) {
        const remainingTime = getFormattedRemainingTime(`login:${email}`)
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (success) {
      ...
      const newRateLimit = checkRateLimit(`login:${email}`)
      if (newRateLimit.isLimited) {
        const remainingTime = getFormattedRemainingTime(`login:${email}`)
        setRateLimitError(`Too many login attempts. Please try again in ${remainingTime}.`)
      } else if (newRateLimit.remainingAttempts < MAX_ATTEMPTS) {
        setError(`Invalid email or password. ${newRateLimit.remainingAttempts} attempt${newRateLimit.remainingAttempts !== 1 ? 's' : ''} remaining.`)
      } else {
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-to-br from-gray-900 via-indigo-900 to-purple-900 items-center justify-center p-12 border-r border-gray-700">
        <div className="text-center text-white">
          ...
          <p className="text-xl text-indigo-100 font-body">Master programming languages with interactive 3D lessons</p>
        </div>
      </div>
      ...
      <div className="w-full lg:w-1/2 flex items-center justify-center bg-gray-900 p-8">
        <div className="w-full max-w-md">
          <div className="bg-gray-800 border border-gray-700 p-8 rounded-lg shadow-xl">
            <h2 className="text-3xl font-bold font-heading mb-2 text-white">Welcome Back</h2>
            <p className="text-gray-400 mb-6 font-body">Sign in to continue your learning journey</p>
